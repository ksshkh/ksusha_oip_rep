...
axes.set_title("Charge-discharge graph in RC-circuit", fontsize = 20)

charge_plot_line   , = axes.plot(charge_data[0], charge_data[1], color = 'yellow')
discharge_plot_line, = axes.plot(discharge_data[0], discharge_data[1], color = 'orange')
...
axes.axvline(x = charge_time, ymin=y_limits[0], ymax = volt_max/y_limits[1], color = 'red', linestyle='dashed')
axes.axhline(y = volt_max, xmin=x_limits[0], xmax = charge_time/x_limits[1], color = 'red', linestyle='dashed')

axes.scatter(time_arr[volt_max_ind], volt_max, color='green')

axes.scatter(x = charge_time, y = 0.0, color='green')
axes.text(x=charge_time+0.1, y = 0.05, s=str(round(charge_time, 1)), fontsize = 10)

axes.scatter(x = 0.0, y = volt_max, color = 'green')
axes.text(x = 0.1, y = volt_max+0.05, s = str(round(volt_max, 1)), fontsize = 10)

axes.text(x = (charge_time/2-0.8), y = volt_max/2, s = ("Charge time: " + str(round(charge_time, 2)) + " s"), color = 'green', fontsize = 12)
axes.text(x = (charge_time+discharge_time/2-0.8), y = volt_max/2, s = ("Discharge time: " + str(round(discharge_time, 2)) + " s"), color = 'pink', fontsize = 12)
